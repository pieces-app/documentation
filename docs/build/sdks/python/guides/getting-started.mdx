...
2. **Install the PyPI Package**: Use pip to install the Pieces SDK package:
    ...
    This will be fixed in the coming weeks but for now, please install the package with the version number.
...
You have successfully connected to the Pieces OS server using the Python SDK. You can now explore the various endpoints available in the SDK to build your application on top of Pieces OS.

Click the `Next →` button below to get started with the Python SDK documentation.
