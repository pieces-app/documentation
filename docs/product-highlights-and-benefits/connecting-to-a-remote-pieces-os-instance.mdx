...
1. Locate your .wslconfig file located at (windows_user_profile)/.wslconfig i.e: 'C:\\Users\\caleb\\.wslconfig'
2. If the .wslconfig file is not present, then create it at the specified path
3. If there is not a section labeled '[wsl2]', create one.
4. Add a line under the [wsl2] section that says: 'networkingMode=mirrored'
5. By the end of these steps your .wslconfig file should look something like:
```
[wsl2]
networkingMode=mirrored
```
6. Restart WSL by running `wsl --shutdown` and `wsl` in a Windows command prompt
