...
The Live Context feature in Pieces enhances the functionality of the Pieces Copilot by utilizing our proprietary Workstream Pattern Engine (WPE) algorithms. This feature is designed with privacy and efficiency in mind, ensuring that all data processing and storage occur locally on your device.
...
The WPE continuously operates in the background, capturing and analyzing user interactions such as clicks and navigation on the desktop. It performs OCR on screenshots to extract textual data, all of which is stored locally. When you navigate to the desktop copilot and engage with it:

- **Local:** If you have a local LLM like Mistral selected, all relevant data queried by the WPE is processed by Mistral entirely on your device.
- **Cloud:** If a cloud-based LLM like GPT-4 is selected, relevant data is sent to GPT-4 for processing.

This system ensures that you can gather real-time context without compromising data privacy, as all context is stored and processed on-device, with additional filtering for secrets, passwords, and other sensitive data.
