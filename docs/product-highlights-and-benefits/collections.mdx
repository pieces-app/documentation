...
1. Collections in Pieces Desktop
    ...
    To get to Collections, press `⌘ + ⇧ + C` or `CTRL + ⇧ +  C` and a new dialog will open. You can also press the `Add snippets +` button on either view, then select the Collection(s) you want. You can add them all at once by pressing the plus button on each language-specific card. Here is an example of one of the cards:
