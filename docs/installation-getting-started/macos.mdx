...
<CTAButton
  ...
  icon={'/assets/pieces_logos/black_circle.png'}
  darkIcon={'/assets/pieces_logos/white_circle.png'}
  type={'secondary'}
/>
<MiniSpacer />
<CTAButton
...
3. Check `Chip` in the window that pops up.
    - If you have an Intel chip, select the `Intel` package above
    - If you have an Apple M1, M2, or M3, select the `Apple Silicon` package.
...
<details>
  ...
  > You can also install Pieces via Homebrew.
...
2. Following the installation, make sure [Pieces OS](/installation-getting-started/pieces-os) is running in the background.
    - Ensure that the Pieces logo appears in your menu bar.
