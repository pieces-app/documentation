...
[What is Pieces OS? How does it work?](<https://docs.pieces.app/faq#what-is-pieces-os-how-does-it-work>)
## PiecesOS
<Tabs defaultValue={"macos"}  values={[{label: 'macOS', value: 'macos'}, {label: 'Windows', value: 'windows'}, {label: 'Linux', value: 'linux'}]}>
 ...
 <TabItem value="windows">
  <CTAButton
      ...
      type={'secondary'}
    />
    <p> Click the PiecesOS icon in your task bar and view in the popup</p>
    <p> If you installed PiecesOS via our Suite Installer, you don't have to manually update the applications nor do the updates within the applications themselves. They will automatically update in the background when restart the applications or your computer</p>
    <p> If you installed the desktop app via the standalone EXE, you will receive a prompt that gives you the option to delay or do the update. Nonetheless, we recommend using the Suite installer instead because the updates are automatic.</p>
    <p> Before you make the switch, uninstall Pieces OS first and then the Pieces for Developers desktop app to prevent conflicts. Your user-generated data will not be affected during this change.</p>
  </TabItem>
  <TabItem value="linux">
    <CTAButton
      href={"/faq/linux"}
      ...
      type={'secondary'}
    />
    <p> To get information, including the version of Pieces OS and Pieces for Developers desktop app snap packages, you can the following commands: </p>
```bash
$ snap info-pieces-os
$ snap info pieces-for-developers
```
<p> This will also show any available updates.</p>
<p> To update Pieces OS and the desktop app, you can run this command: </p>
```bash
$ sudo snap refresh
```
  </TabItem>
...
### Pieces for VS Code
<p> Select Extensions, search Pieces for VS Code, and you’ll see the version next to the title.</p>

### Pieces for JetBrains
<p> Select Settings > Plugins…> search Pieces > see version</p>

### Pieces for Obsidian
<p> Select Settings > Community Plugins > see version under Pieces for Developers</p>

### Pieces for Jupyterlab
<p> Select Extension Manager > see version next to jupyter-pieces</p>
