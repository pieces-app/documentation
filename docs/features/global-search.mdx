...
To access Global Search in the Pieces Desktop App, select ` Copilot & Global Search ` from the dropdown menu in the upper left corner where other views like [Workflow Activity](/docs/features/workflow-activity) and [Snippet Discovery](/docs/product-highlights-and-benefits/in-project-snippet-discovery) live.
...
1. The snippets that match your results will be in vertical cards containing:
    ...
    - A related link to the source that the resource came from, e.g., Saved using the [Chrome Extension](/docs/extensions-plugins/chrome)
