...
### qGPT
- Make sure POS is open, and updated to at least 5.1.2
- To view the Copilot, open the pieces plugin and click the bot logo on the top slider to switch to Copilot
- The copilot will traverse the directory you launched JupyterLab from in order to load in your notebooks as context for the chatbot, this means if you have multiple notebooks in that same directory, it will load in all of them for you to query.
...
- Each response from the Copilot will also contain the relevant notebooks that it was able to gain context from in order to answer your question, click on one of those buttons in order to open that notebook in your editor.

### Enrich Selection
- With the new release, you can also select any text within a code cell in your notebook, right click, and add a useful description to it.

### Realtime Streaming
- We added a websocket connection with this release as well. This means that all the changes you would make across any Pieces integration (i.e PFD, VSCode, Jetbrains, etc) should automatically be reflected in the plugin.
