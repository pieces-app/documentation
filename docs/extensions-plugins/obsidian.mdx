...
To create a filter and show snippets based on a set of parameters, you can manually add filtered settings to your snippet list. To add a filter based on tags, titles, content and more, follow these steps here:
...
You can select your specific Pieces Copilot runtime from inside of the Pieces Obsidian Plugin, similar to how this works in other extensions when selecting the appropriate model for your environment. You have options between cloud models and also local models. To select a model try these steps:
...
3. Click on the corresponding model that you would like to use
4. Download (if a local model) which model you would like to use. Make sure your system has the right hardware to satisfy the requirements listed here.
    - The model will be downloaded as a 4GB file, so it may take some time depending on your internet connection.
5. Once the model has been downloaded, there should be a plug icon, click on the plug icon to select the model.
