...
def fix_h1_headers(text):
    ...
    pattern = r'title: core_openapi\.api\.(\w+)Api'
    ...
    pattern = r'title: core_openapi\.model\.(\w+)'
...
def fix_str_links(text):
    pattern = r'\[(.*?)\]\(../models/String\)'
    replacement = r'\1'
    result = re.sub(pattern, replacement, text)
    return result


def fix_date_links(text):
    pattern = r'\[(.*?)\]\(DateTime\)'
    replacement = r'\1'
    result = re.sub(pattern, replacement, text)
    return result
...
def organize_markdown_files_in_directory(source_directory):
    ...
    for file_path in file_paths:
        ...
        transformed_content = fix_types(transformed_content)
...
def main():
    root_directory = './docs/build/reference/dart_raw'
